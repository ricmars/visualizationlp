...
- When toggling flags (primary) or changing options/order/labels on existing fields, you MUST include for each field: id, name, type, objectid, label, and sampleValue. If any of these are missing from context, call listFields(objectid) ONCE to retrieve them, then perform a single saveFields call for all targets.
...
- Always create a complete starter workflow object (hasWorkflow=true), not just fields, with at least 4 stages, should have at least 4 stages, each with at least 3 steps including 2 steps of type "Collect information"; use a mix of step types. Any "Collect information" step must set viewId to one of the created views. Use integer IDs and consistent ordering for stages/processes/steps.
